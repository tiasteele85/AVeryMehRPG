<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
   pageEncoding="ISO-8859-1"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<!DOCTYPE html>
<html>
   <head>
      <link rel="stylesheet" type="text/css" href="bootstrap.css">
      <link rel="stylesheet" type="text/css" href="newstyle.css">
      <link href="https://fonts.googleapis.com/css?family=Abril+Fatface|Anton|Lexend+Zetta&display=swap" rel="stylesheet">
      <meta charset="UTF-8">
      <title>Knighty Knight</title>
   </head>
   <body>
      <!--audio autoplay>
         <source src="http://ccmixter.org/content/cyba/cyba_-_new_circle.mp3">
         </audio-->
         <br>
      <h1 class="display-4" style="font-family: 'Abril Fatface', cursive">I don't think so Knighty McKnighterson!</h1>
      <div class="container" style="font1">
         <div class="row">
            <div class="col-6">
               <label id="font1">
                  <p>The Knight, infuriated that you stole his Dagger, exclaims "I will reclaim my dagger off your dead body, scoundrel!"
                  </p>
                  <br>
                  <br>
                  <br>
                  <a href="/fight">
                  <button type="button" class="btn btn-primary btn-lg">Defend Yourself!</button>
                  </a>
               </label>
            </div>
            <div class="col-5">
               <label>
               <img border="5px" src="roadside.jpg" alt="roadside" width="500" height="350"></img>
               </label>
            </div>
            <div class="col-3">
               <label>
               </label>
            </div>
            <div class="col-3">
               <label>
               </label>
            </div>
         </div>
      </div>
      <br>
      <br>
   </body>
</html>